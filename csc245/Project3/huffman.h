...
// Comparison struct/function for the priority queue
struct CompareNodes {
    bool operator()(const HNode& node1, const HNode& node2) {
        return node1.weight > node2.weight; // Min-heap based on weights
    }
};
...
class HuffmanTree {
	...
	void insert(char ch, int weight);	
		// inserts a character and its associated weight
		// into a node of the Huffman tree

	void insert(char ch);
        // inserts a character and self-adjusts weight
		// into a node of the Huffman tree
	...
	string GetCode(char ch);
		// returns the compressed code of a given
		// character in the tree using recursion

	void PrintTable();	
		// displays the Huffman encoding table used in
		// the compression process
	
	int numNodes();	
		// returns the number of nodes currently in Huffman tree
	...
	string GetCode(int i);			
		// returns the compressed code of the character at 
		// a given index
};
